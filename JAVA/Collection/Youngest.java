package Collection;

import java.util.ArrayList;

public class Youngest {
    public static void main(String args[]) {
        ArrayList<Employee> list = new ArrayList<>();
		list.add(new Employee(1,"Aman",21,"M","HR",2020,140.80));
		list.add(new Employee(2,"Rahul",21,"M","IT",2021,132.50));
		list.add(new Employee(3,"Sumit",26,"M","Purchase",2022,139.90));
		list.add(new Employee(4,"Vaishnavi",25,"F","Operations",2020,100.05));
        
        int minAge = 100;
        String nameEmp="";
        for(int i=0;i<list.size();i++){
            Employee obj = list.get(i);
            if(obj.getAge() < minAge && obj.getDepartment().equals("Product")){
                minAge = obj.getAge();
                nameEmp = obj.getName();
            }
        }
        System.out.println("The Youngest Employee of Product development department is "+nameEmp+" and age is "+minAge);
    }
}
